...
if (useCookie) {
    try {
        if (fs.existsSync("cookie.txt")) {
            let rawCookies = fs.readFileSync("cookie.txt", "utf8").trim();
            let jsonCookies = JSON.parse(rawCookies);
            cookies = Object.entries(jsonCookies).map(([key, value]) => `${key}=${value}`).join('; ');
            console.log("Đã tải cookie từ cookie.txt và chuyển đổi thành chuỗi hợp lệ");
        } else if (fs.existsSync("cookie.json")) {
            let jsonData = JSON.parse(fs.readFileSync("cookie.json", "utf8"));
            cookies = jsonData.cookie || "";
            console.log("Đã tải cookie từ cookie.json");
        } else {
            console.log("Không tìm thấy file cookie.txt hoặc cookie.json, tiếp tục không sử dụng cookie...");
        }
    } catch (err) {
        console.error("Lỗi đọc file cookie:", err);
    }
}
let ratelimit = [];
